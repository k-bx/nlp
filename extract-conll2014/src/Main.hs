...
import Data.Conduit (Consumer, Sink, ($$), ConduitT)
...
parseMistake :: MonadThrow m => ConduitT Event o m (Maybe Annotation)
parseMistake =
  tag' "MISTAKE" (requireAttr "start_par") $ \startPar -> do
    mistakeType <- tag' "TYPE"

parseAnnotation :: MonadThrow m => ConduitT Event o m (Maybe Annotation)
parseAnnotation =
  tag' "ANNOTATION" (requireAttr "teacher_id") $ \teacherId -> do
